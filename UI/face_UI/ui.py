...
from utils.process_video import process_frame
...
class UI_main(QMainWindow):
    ...
    def stop_recording(self):
        if self.recording:
            ...
            # 重置视频录制区域
            self.ui.video_label.setPixmap(QPixmap())  # 清空视频显示区域
    ...
    def use_ocr_or_input(self):
        ...
        if self.ui.checkBox.isChecked():
            ...
            if not self.current_id:
                self.show_label_2("No valid ID found in input.", color="red")
                self.cap.release()
                return
            ...
            if not student_id:
                self.show_label_2("Failed to recognize Student ID.", color="red")
                self.cap.release()
                return

            self.current_id = self.extract_student_id(student_id)
            if not self.current_id:
                self.show_label_2("No valid Student ID found.", color="red")
                self.cap.release()
                return
